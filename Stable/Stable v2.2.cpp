...
void sysinfo()
{
	...
	cout << "**************************" << endl;
	cout << "版本：Stable v2.2 构建时间：2023-01-05" << endl;
}
double cal(double x , double y , char op)
{
	...
	if(y == 0 && op == '/')
	{
		MessageBox(NULL , "不可以除以0！" , "错误" , MB_OK+16);
		return 0;
	}
	switch op:
	{
		...
		default:
		{
			MessageBox(NULL , "运算未知。" , "错误" , MB_OK+16);
			return 0;
		}
	}
	return 0;
...
int main()
{
	...
	while(1)
	{
		...
		switch(ope)
		{
			case 1:
			{
				cout << endl << "计算器使用注意事项：" << endl << "1.在做除法时，请保证第一个数大于后面的。" << endl << "2.除法的结果保留三位小数。" << endl;
				double x , y; char op;
				cout << "请输入两个数：";
				cin >> x >> y;
				cout << "请输入一下操作之一：+（加）、-（减）、*（乘）、/（除）、%（求余）：";
				cin >> op;
				printf("结果是：%.3lf\n" , cal(x , y , op));
				break;
			}
			...
			{
				cout << "更新日志：" << endl;
				cout << "更新了计算模块的判读逻辑，现在多重选择分支已经全面使用switch...case语句。" << endl;
				break;
			}
			case 7:
			{
			...
			default:
			{
				MessageBox(NULL , "输入无效。请重试。" ,  "错误！" , MB_OK+16);
				break;
			}
			cout << endl;
		}
